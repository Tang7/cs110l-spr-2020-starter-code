    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                let val2 = grid.get(i, j + 1).unwrap();
                let _ = grid.set(i + 1, j + 1, val1.max(val2));
            }
        }
    }
...
fn main() {
    ...
    let seq1 = read_file_lines(filename1).expect("read filename1 failed");
    let seq2 = read_file_lines(filename2).expect("read filename2 failed");
