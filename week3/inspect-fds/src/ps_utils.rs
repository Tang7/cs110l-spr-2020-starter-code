...
fn get_process(pid: usize) -> Result<Option<Process>, Error> {
    ...
    let output = String::from_utf8(
        Command::new("ps")
            .args(&[&pid.to_string(), "-o", "pid= ppid= command="])
            .output()?
            .stdout,
    )?;
    ...
    if output.trim().len() > 0 {
        ...
        println!("Failed parse line from ps command");
        Ok(None)
    }
}
...
fn get_pid_by_command_name(name: &str) -> Result<Option<usize>, Error> {
    let output = String::from_utf8(Command::new("pgrep").args(&[name]).output()?.stdout)?;
...
pub fn get_target(query: &str) -> Result<Option<Process>, Error> {
    ...
    if pid_by_command.is_some() {
        println!("returning process with pid {}", pid_by_command.unwrap());
